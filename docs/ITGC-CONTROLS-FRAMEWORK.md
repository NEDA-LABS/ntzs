...
1. **Vendor risk & contracts.** Risk assessments and **DPAs/SLAs with breach-notification clauses** for Neon, Vercel, Fly.io, AWS, Alchemy, Snippe, AzamPay, Resend, and Selcom (identity verification as well as payments). Track vendor SOC 2 reports. Where a partner performs customer due diligence on our behalf, the **signed reliance agreement** is the control document: it must oblige them to verify to our standard and to produce the underlying records on request. See `11-KYC-RELIANCE-POLICY.md` (`NEDA-CDD-REL-2026-01`). Reliance is granted per partner in Backstage and recorded on the partner row with the agreement reference.
