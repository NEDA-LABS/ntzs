...
1. **Vendor risk & contracts.** Risk assessments and **DPAs/SLAs with breach-notification clauses** for Neon, Vercel, Fly.io, AWS, Alchemy, Snippe, AzamPay, Resend, and Selcom (identity verification as well as payments). Track vendor SOC 2 reports. Where customer due diligence is performed outside the NIDA registry path, `11-KYC-RELIANCE-POLICY.md` (`NEDA-CDD-REL-2026-01`) is the control document: it sets the verification standard, record retention, production on demand, and quarterly assurance sampling. Today that work is performed in the NEDApay onboarding process — the same legal entity, so no third-party reliance arises; a grant to any separate entity additionally requires an executed agreement and a supervision assessment. Grants are made per partner in Backstage and recorded on the partner row against the policy reference.
