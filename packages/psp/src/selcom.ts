 ...
 * (b) being onboarded as the consolidated PSP. This module targets Selcom's
 * **Business API** (https://developer.selcom.business), which as of Jun 2026
 * covers the OFF-RAMP + reconciliation surface:
 ...
 *   - balance                         → getBalance
 *   - statement                       → getStatement (for reserve reconciliation)
 *
 * The ON-RAMP (collections: push-USSD + control-number) is still in development
 * on Selcom's side (per Rosario Arun, 22 Jun 2026). initiatePayment /
 * initiateCardPayment / checkPaymentStatus are therefore STUBS that throw until
 * Selcom ships the collections endpoints and we confirm the surface + auth.
 *
 ...
 *   fixed-IP gateway/proxy in production.
 ...
 *
 * ⚠ EVERY endpoint path, field name, FI code, purpose code and response shape
 * below is taken from the published docs and MUST be verified against Selcom's
 * sandbox before going live. Items needing confirmation are flagged with ⚠.
 *
 * Docs: https://developer.selcom.business/#authorization
 */
 ...
 */
function signRequest(fields: SignedField[]): {
...
const MOBILE_FI_CODES: Record<string, string> = {
  vodacom: 'VMCASHIN', // Vodacom M-Pesa — 74/75/76
  airtel: 'AMCASHIN', //  Airtel Money — 68/69/78
  tigo: 'TPCASHIN', //    Mixx by Yas (ex Tigo Pesa) — 71/65/67
  halotel: 'HPCASHIN', // Halo Pesa — 62
  ttcl: 'TTCASHIN', //    TTCL Pesa — 73
}

function detectWalletFiCode(normalizedPhone: string): string {
  ...
  if (['71', '65', '67'].includes(prefix)) return MOBILE_FI_CODES.tigo
  if (['62'].includes(prefix)) return MOBILE_FI_CODES.halotel
...
// ─────────────────────────────────────────────────────────────────────────────
// OFF-RAMP (disbursement / redemption) — LIVE on the Business API
// ─────────────────────────────────────────────────────────────────────────────
...
// The published Selcom send-money tariff lives in the client-safe shared fee
// model (./fees.ts) so UI components can quote it without pulling in crypto.
// Re-exported here for backward compatibility.
import { estimateSendMoneyFee } from './fees'
...
// ─────────────────────────────────────────────────────────────────────────────
// ON-RAMP (collections / deposit) — push-USSD, LIVE on the NEDA sandbox
