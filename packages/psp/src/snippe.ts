...
/**
 * Snippe's standard response envelope (fields vary per endpoint). Typed
 * explicitly because stricter tsconfigs (the worker's) type `json()` as
 * `unknown`.
 */
interface SnippeEnvelope {
  status?: string
  message?: string
  error_code?: string
  data?: {
    ...
    available?: number | { value?: number }
    pending?: number | { value?: number }
    currency?: string
  }
}
...
export async function initiatePayment(
  ...
  try {
    ...
    const result = (await response.json()) as SnippeEnvelope
...
export async function getBalance(): Promise<SnippeBalanceResponse> {
  ...
  // Snippe returns either a bare number or a { value } object per field.
  const avail = result.data.available
  const pend = result.data.pending
  return {
    available: (typeof avail === 'object' && avail !== null ? avail.value : avail) ?? 0,
    pending: (typeof pend === 'object' && pend !== null ? pend.value : pend) ?? 0,
