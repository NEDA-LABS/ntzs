...
('INC-2026-08-001',
 ...
 'Same day. Selcom wallet destination codes corrected from the provider''s published table and proven live per network before volume (1,000 TZS probes received on a Vodacom and an Airtel wallet); production dispatch switched to routed failover (Selcom primary, Snippe fallback). With both providers'' records confirming zero dispatches, all six stranded burns were minted back to the source wallets through the audited operator action, and the operator confirmed the users whole. A live 5,000 TZS user withdrawal then completed end-to-end over the new primary rail the same afternoon (Selcom confirmation SB0801M531U, recipient credited and notified), verifying the restored path in production. The account flag has been raised with Snippe, which now serves as the redundancy rail.',
 ...
 'PRs #206–#213; provider refusal "your account has been flagged for review" persisted on the six burn rows; Snippe dashboard, 1 Aug (zero payouts); Selcom e-statement, 1 Aug (sole debit: the 1,000 TZS probe); Selcom error 651 on legacy code VMCASHIN; probe ref 16437765-f972-49c4-9231-74f09d815298 with confirmation SMS SB0801M4TAC (Vodacom); end-to-end verification withdrawal confirmation SB0801M531U (5,000 TZS, Vodacom); audit_logs action burn.operator_force_reverted — six entries, each carrying its remint tx hash; docs/psp/selcom-destination-shortcodes.md')
