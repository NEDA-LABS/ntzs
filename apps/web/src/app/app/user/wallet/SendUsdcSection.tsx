...
interface SendUsdcSectionProps {
  walletAddress: string
}

export function SendUsdcSection({ walletAddress }: SendUsdcSectionProps) {
  const [open, setOpen] = useState(false)
  const [to, setTo] = useState('')
  const [amount, setAmount] = useState('')
  const [result, setResult] = useState<SendUsdcResult | null>(null)
  const [isPending, startTransition] = useTransition()
  const formRef = useRef<HTMLFormElement>(null)

  useEffect(() => {
    const onOpen = () => { reset(); setOpen(true) }
    window.addEventListener('wallet:openSendUsdc', onOpen)
    return () => window.removeEventListener('wallet:openSendUsdc', onOpen)
  }, [])
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    startTransition(async () => {
      const res = await sendUsdcAction(fd)
      setResult(res)
    })
  }
          ...
          >
            ...
            {result?.success ? (
              <div className="space-y-5 text-center">
                <div className="mx-auto flex h-14 w-14 items-center justify-center rounded-full bg-blue-500/15 ring-1 ring-blue-500/25">
                ...
                <div>
                  ...
                  <p className="mt-1 text-sm text-zinc-400">
                    {result.amount.toLocaleString(undefined, { maximumFractionDigits: 6 })} USDC
                    {' '}sent to{' '}
              ...
              <form ref={formRef} onSubmit={handleSubmit} className="space-y-4">
                <div className="flex items-center justify-between">
                  <h2 className="text-base font-bold text-white">Send USDC</h2>
                ...
                <div className="flex items-center gap-1.5 rounded-xl bg-blue-600/10 px-3 py-2 ring-1 ring-blue-600/20">
                  ...
                  <span className="text-[11px] font-medium text-blue-400">Base network only — sends native USDC on Base</span>
                </div>

                <div className="space-y-1.5">
                  <label className="block text-xs font-medium text-zinc-500">To (0x address)</label>
                  <input
                    type="text"
                    value={to}
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="flex items-center gap-2 rounded-2xl border border-white/[0.07] bg-black/30 px-4 py-3 focus-within:border-blue-500/50 focus-within:ring-1 focus-within:ring-blue-500/30">
                    ...
                    />
                    <span className="text-xs font-semibold text-blue-400">USDC</span>
                  </div>
                </div>
