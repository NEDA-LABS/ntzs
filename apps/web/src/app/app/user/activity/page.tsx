...
export default async function ActivityPage() {
  ...
  return (
    <div className="min-h-screen px-4 pt-4 pb-24 lg:px-8 lg:pt-6">
      ...
      <div className="mb-5 grid grid-cols-3 gap-3">
        <div className="rounded-2xl border border-border/40 bg-card/60 p-4 backdrop-blur-2xl">
          <p className="text-[10px] font-medium uppercase tracking-wider text-muted-foreground">Total In</p>
          <p className="mt-2 text-base font-bold text-emerald-400">
            {totalIn.toLocaleString()}
          </p>
          <p className="mt-0.5 text-[10px] text-muted-foreground">TZS</p>
        </div>
        <div className="rounded-2xl border border-border/40 bg-card/60 p-4 backdrop-blur-2xl">
          <p className="text-[10px] font-medium uppercase tracking-wider text-muted-foreground">Total Out</p>
        ...
        <div className="rounded-2xl border border-border/40 bg-card/60 p-4 backdrop-blur-2xl">
          ...
          <p className="mt-0.5 text-[10px] text-muted-foreground">deposits</p>
        </div>
      </div>

      {/* Transaction list with filter tabs */}
      {txns.length === 0 ? (
        <div className="rounded-2xl border border-border/40 bg-card/60 py-16 text-center backdrop-blur-2xl">
          <div className="mx-auto mb-3 flex h-12 w-12 items-center justify-center rounded-2xl border border-border/40 bg-background/35">
          ...
          <p className="text-sm font-medium text-foreground/80">No transactions yet</p>
          <p className="mt-1 text-xs text-muted-foreground">Make your first deposit to get started</p>
        </div>
      ) : (
