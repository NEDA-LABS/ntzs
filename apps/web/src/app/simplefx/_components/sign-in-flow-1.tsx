...
export const SignInPage = ({ className }: SignInPageProps) => {
  ...
  const handleEmailSubmit = async (e: React.FormEvent) => {
    ...
    setAuthError("");
    try {
      const res = await fetch("/simplefx/api/auth/request-otp", {
        method: "POST",
  ...
  const handleCodeChange = async (index: number, value: string) => {
    if (value.length <= 1) {
      ...
      if (index === 5 && value && newCode.every((d) => d.length === 1)) {
        ...
        setAuthError("");
        try {
          const res = await fetch("/simplefx/api/auth/verify-otp", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
                ...
                {step === "email" ? (
                  ...
                  >
                    ...
                    <p className="text-xs text-gray-600 pt-2">
                      ...
                      <Link href="/simplefx/terms" className="underline text-gray-500 hover:text-gray-300 transition-colors">
                        Terms of Service
                      </Link>{" "}
                      and{" "}
                      <Link href="/simplefx/privacy" className="underline text-gray-500 hover:text-gray-300 transition-colors">
                        Privacy Policy
                      </Link>
                      .
                    </p>
                  ...
                  >
                    ...
                    <motion.p
                      ...
                      onClick={async () => {
                        setAuthError("");
                        try {
                          await fetch("/simplefx/api/auth/request-otp", {
                            method: "POST",
                            headers: { "Content-Type": "application/json" },
                            body: JSON.stringify({ email }),
                          });
