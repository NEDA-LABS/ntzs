  ...
  <div className="relative w-full h-full flex items-center justify-center">
    <div className="absolute" style={{ left: '50%', top: '50%', transform: 'translate(-72%, -50%)' }}>
      <div className="w-28 h-28 rounded-full bg-zinc-900/80 border border-white/10 flex items-center justify-center shadow-xl">
        <img src="/usdt-coin.svg" alt="USDT" className="w-20 h-20 object-contain drop-shadow-[0_0_20px_rgba(38,195,135,0.5)]" draggable={false} />
      </div>
    </div>
    <div className="absolute" style={{ left: '50%', top: '50%', transform: 'translate(-28%, -50%)' }}>
      <div className="w-32 h-32 rounded-full bg-zinc-900/80 border border-white/10 flex items-center justify-center shadow-xl">
        <img src="/usdc-logo.svg" alt="USDC" className="w-24 h-24 object-contain drop-shadow-[0_0_24px_rgba(59,130,246,0.55)]" draggable={false} />
      </div>
    </div>
  </div>
);
...
const OrbitItem = ({ label, icon, radius, duration, initialAngle }: OrbitBadge) => (
  <motion.div
    ...
    style={{ position: 'absolute', top: '50%', left: '50%', width: 0, height: 0, transformOrigin: '0px 0px' }}
  >
    <motion.div
      animate={{ rotate: [-(initialAngle), -(initialAngle + 360)] }}
      transition={{ duration, repeat: Infinity, ease: 'linear' }}
      style={{ position: 'absolute', top: -radius, transform: 'translateX(-50%)' }}
    >
...
const TokenVisual = ({ data, isLeft }: { data: ShowcaseItem; isLeft: boolean }) => (
  <div className="relative shrink-0 flex items-center justify-center">
    <motion.div animate={{ rotate: 360 }} transition={{ duration: 24, repeat: Infinity, ease: 'linear' }} className={`absolute inset-[-18%] rounded-full border border-dashed ${data.colors.ring}`} />
    <motion.div animate={{ rotate: -180 }} transition={{ duration: 36, repeat: Infinity, ease: 'linear' }} className={`absolute inset-[-8%] rounded-full border border-dotted ${data.colors.ring} opacity-40`} />
    <motion.div animate={{ scale: [1, 1.06, 1] }} transition={{ duration: 4, repeat: Infinity, ease: 'easeInOut' }} className={`absolute inset-0 rounded-full bg-gradient-to-br ${data.colors.gradient} blur-2xl opacity-30`} />
    <div className="relative h-56 w-56 md:h-72 md:w-72 rounded-full border border-white/5 shadow-2xl flex items-center justify-center overflow-hidden bg-black/20 backdrop-blur-sm">
      <motion.div animate={{ y: [-8, 8, -8] }} transition={{ repeat: Infinity, duration: 5, ease: 'easeInOut' }} className="w-full h-full">
        <AnimatePresence mode="wait">
          <motion.div key={data.id} variants={logoVariants(isLeft)} initial="initial" animate="animate" exit="exit" className="w-full h-full">
    ...
    <div className="absolute -bottom-7 left-1/2 -translate-x-1/2 whitespace-nowrap">
      <div className="flex items-center gap-1.5 text-xs uppercase tracking-widest text-zinc-500 bg-zinc-950/80 px-3 py-1.5 rounded-full border border-white/5 backdrop-blur">
        <span className="h-1.5 w-1.5 rounded-full animate-pulse" style={{ backgroundColor: data.id === 'ntzs' ? 'rgb(52,211,153)' : 'rgb(59,130,246)' }} />
        {data.status}
      </div>
    </div>
  </div>
...
const TokenDetails = ({ data }: { data: ShowcaseItem }) => (
  <motion.div variants={containerVariants} initial="hidden" animate="visible" exit="exit" className="flex flex-col items-start text-left">
    ...
    <motion.h3 variants={contentVariants} className="text-3xl md:text-4xl font-bold tracking-tight mb-3 text-transparent bg-clip-text bg-gradient-to-b from-white to-zinc-500">
      {data.title}
    </motion.h3>
    <motion.p variants={contentVariants} className="text-zinc-400 text-sm leading-relaxed mb-6 max-w-sm">
      {data.description}
    </motion.p>
    <motion.div variants={contentVariants} className="w-full space-y-5 bg-zinc-900/40 p-5 rounded-2xl border border-white/5 backdrop-blur-sm">
      {data.metrics.map((metric, idx) => (
        <div key={metric.label}>
          ...
          <div className="relative h-1.5 w-full bg-zinc-800 rounded-full overflow-hidden">
            <motion.div initial={{ width: 0 }} animate={{ width: `${metric.value}%` }} transition={{ duration: 1, delay: 0.3 + idx * 0.15, ease: 'easeOut' }} className={`absolute top-0 bottom-0 ${data.colors.bar} opacity-80 rounded-full`} />
          </div>
        </div>
      ))}
      ...
      <button type="button" className="flex items-center gap-2 text-xs font-semibold uppercase tracking-wider text-zinc-400 hover:text-white transition-colors group">
    ...
    <section id="how-it-works" className="relative w-full bg-black overflow-hidden py-24 px-6">
      <div className="absolute inset-0 pointer-events-none opacity-20">
        <CanvasRevealEffect
          ...
          colors={[[147, 197, 253], [165, 180, 252]]}
          dotSize={3}
        />
      </div>
      <div
      ...
      <div className="relative z-10 max-w-7xl mx-auto">
        <div className="text-center mb-16">
          ...
          <h2 className="text-4xl md:text-5xl font-thin tracking-tight text-white mb-4">The Spread is the Yield</h2>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_auto_1fr] items-start gap-8 lg:gap-6">
          <motion.div initial={{ opacity: 0, x: -48 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true, margin: '-80px' }} transition={{ duration: 0.7, ease: [0.16, 1, 0.3, 1] }} className="flex flex-col items-center gap-12">
          ...
          <div className="hidden lg:flex flex-col items-center justify-center self-stretch gap-4 px-2 pt-24">
            ...
            <div className="flex flex-col items-center gap-2 py-4">
              ...
              <div className="px-3 py-1.5 rounded-full border border-white/8 bg-zinc-900/60 text-zinc-400 text-xs font-mono">1.5%</div>
              <ArrowRight size={14} className="text-zinc-700 rotate-90" />
              <ArrowRight size={14} className="text-zinc-700 -rotate-90 -mt-2" />
              <div className="px-3 py-1.5 rounded-full border border-white/8 bg-zinc-900/60 text-zinc-400 text-xs font-mono">1.2%</div>
              <div className="text-[10px] uppercase tracking-widest text-zinc-600">Bid</div>
            </div>
            <div className="flex-1 w-px bg-gradient-to-b from-transparent via-white/10 to-transparent" />
          </div>
          <motion.div initial={{ opacity: 0, x: 48 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true, margin: '-80px' }} transition={{ duration: 0.7, ease: [0.16, 1, 0.3, 1], delay: 0.1 }} className="flex flex-col items-center gap-12">
