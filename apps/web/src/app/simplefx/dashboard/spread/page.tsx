...
function BpsSlider({ label, value, onChange, color }: { label: string; value: number; onChange: (v: number) => void; color: 'blue' | 'indigo' }) {
    ...
    <div className="rounded-xl border border-white/5 bg-zinc-950 p-5">
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-baseline gap-1">
          <span className={`text-2xl font-light tabular-nums ${color === 'blue' ? 'text-blue-400' : 'text-indigo-400'}`}>{pct}</span>
          <span className="text-xs text-zinc-600">%</span>
          <span className="text-xs text-zinc-600 ml-1">({value} bps)</span>
        </div>
      </div>
      <input type="range" min={10} max={500} step={5} value={value} onChange={(e) => onChange(Number(e.target.value))} className="w-full accent-blue-500 cursor-pointer" />
  ...
  return (
    <div className="px-6 py-8 max-w-2xl mx-auto">
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
        ...
        <p className="text-zinc-500 text-sm mb-8">Set how much you earn on each side of every nTZS swap. Higher spread = more earnings per fill, fewer fills.</p>
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-zinc-500">
                {lp.isActive ? 'Orders are filling against your inventory in real time.' : 'Activate to start receiving order fills automatically.'}
              </p>
            </div>
            <button
              ...
              className={`flex items-center gap-2 px-4 py-2 rounded-full text-sm font-medium transition-all ${
                lp.isActive ? 'bg-zinc-800 text-zinc-400 hover:bg-zinc-700' : 'bg-gradient-to-br from-blue-500 to-blue-700 text-white hover:from-blue-400 hover:to-blue-600'
              }`}
            >
              <Zap size={14} />
