...
export async function POST(req: NextRequest) {
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    if (!r.amountTzs || isNaN(r.amountTzs) || r.amountTzs <= 0)
      rowErrors.push({ row: rowNum, error: 'Invalid amount' })
    if (r.amountTzs > MAX_PER_TXN_TZS)
      rowErrors.push({
        row: rowNum,
        error: `Amount TZS ${r.amountTzs.toLocaleString()} exceeds BoT per-transaction limit of TZS 1,000,000`,
      })
    if (r.payoutMethod === 'eft' && !r.bankAccount?.trim())
      rowErrors.push({ row: rowNum, error: 'Bank account required for EFT payout' })
  }
