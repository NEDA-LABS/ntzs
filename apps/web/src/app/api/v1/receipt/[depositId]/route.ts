...
// Public — no auth required. Only resolves for completed (minted) payments.
export async function GET(
  ...
  return NextResponse.json({
    ...
    merchantBusinessName: row.merchantBusinessName,
    merchantHandle: row.merchantHandle,
    productName: row.productName,
    linkDescription: row.linkDescription,
    pspReference: row.pspReference,
    pspChannel: row.pspChannel,
