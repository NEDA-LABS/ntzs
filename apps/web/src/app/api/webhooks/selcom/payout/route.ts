 ...
 * is an accelerator — the reconcile poller is the primary completion/failure
 * detection path (joins poll-selcom at disbursement go-live, when the
 * payout_provider stamp from drizzle/0061 is adopted).
 *
...
export async function POST(request: NextRequest) {
  ...
  // Look up by OUR stamped reference (the transId we generated at payout).
  const [burn] = await db
    .select({
      id: burnRequests.id,
    ...
    .from(burnRequests)
    .where(eq(burnRequests.payoutReference, referenceId))
    .limit(1)

  if (!burn) {
    console.warn('[selcom/payout webhook] no burn for reference', { referenceId })
  ...
  if (confirmed.status === 'completed') {
    ...
    // from a webhook context — the admin reconcile path completes the revert
    // with a full paper trail.
    await db
      .update(burnRequests)
      .set({
        payoutStatus: 'reconcile_required',
        payoutError: confirmed.failureReason ?? 'Payout failed (confirmed by query)',
        updatedAt: new Date(),
      })
      .where(and(eq(burnRequests.id, burn.id), eq(burnRequests.payoutStatus, 'pending')))
